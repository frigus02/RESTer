...
async function main() {
    ...
    // - Dark (default) DEV
    await writeFile(rootDir + '/.icons/icon-dev.svg', builder.buildObject(iconXml), { encoding: 'utf8' });

    // - Light DEV
    iconXml.svg.path.forEach(path => {
        path.$.style = path.$.style.replace('fill:#0c0c0d', 'fill:#f9f9fa');
    });
    await writeFile(rootDir + '/.icons/icon-light-dev.svg', builder.buildObject(iconXml), { encoding: 'utf8' });

    // - Light
    const devGroupIndex = iconXml.svg.g.findIndex(node => node.$.id === 'dev');
    iconXml.svg.g.splice(devGroupIndex, 1);
    await writeFile(rootDir + '/.icons/icon-light.svg', builder.buildObject(iconXml), { encoding: 'utf8' });

    // - Dark (default)
    iconXml.svg.path.forEach(path => {
        path.$.style = path.$.style.replace('fill:#f9f9fa', 'fill:#0c0c0d');
    });
    await writeFile(rootDir + '/.icons/icon.svg', builder.buildObject(iconXml), { encoding: 'utf8' });

    // Export PNGs
    for (const variation of variations) {
        for (const size of sizes) {
            const inputSvg = path.resolve(`${rootDir}/.icons/icon${variation}.svg`);
            const outputPng = path.resolve(`${rootDir}/src/images/icon${variation}${size.px}.png`);
            await exec([
