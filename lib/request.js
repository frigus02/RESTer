...
function generateFormData(body, variableValues) {
    const rawData = decodeQueryString(body),
          formData = Cc['@mozilla.org/files/formdata;1'].createInstance(Ci.nsIDOMFormData);
...
function readBase64CustomObjectAsBlob(obj) {
    const bytes = base64.toByteArray(obj.data);
    ...
    return new Promise(function (resolve, reject) {
        try {
            ...
            if (contentType && contentType.value === 'multipart/form-data') {
                ...
                request.body = generateFormData(request.body, request.variables.values);
            }
